...
"""
function random_angles_on_cap(theta_max; theta_min = 0., phi_min = 0., phi_max = 2*pi)
...
"""
function random_direction(normal = SA[0., 0., 1.0])

    # check that normal is accurately normalized
    @toggled_assert norm(normal) ≈ 1.0

    # draw a random polar angle uniformly in solid angle
    theta = acos(sqrt(rand(Uniform(0., 1.))))
