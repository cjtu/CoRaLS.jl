...
"""
acceptance(ntrials, nbins, orbit, target; ...)

Acceptance run including orbits and target locations.

"""
function acceptance(ntrials::Integer, nbins::Integer, target::Target, orbit::AbstractOrbit; 
    min_energy=0.1EeV,
    max_energy=600.0EeV,
    altitude=20.0km,
    ...
    @showprogress 1 "Simulating..." for bin = 1:nbins
        ...
        # sample orbital positions
        SCs = sample_orbit(orbit, ntrials)
...
function acceptance(ntrials, nbins; min_energy=0.1EeV,
    ...
    return Acceptance(ntrials, altitude, AΩ, energies, dAΩ, devents, rAΩ, revents, Nothing, Nothing)
