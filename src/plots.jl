...
"""
    plot_incident_angles(AΩ)

Plot the incident angles at the ice and the surface, along with
the corresponding Fresnel coefficients.
"""
function plot_incident_angles(AΩ)
    ...
    axes[1].set(xlim=[0, 60.], ylim=[1e-5, 0.9])
    axes[1].set_xticks([0, 15., 30., 45., 60.])
    axes[2].set(xlim=[0, 60.], ylim=[1e-5, 0.9])
    axes[2].set_xticks([0, 15., 30., 45., 60.])
    [rax.set_ylim([0., 1.1]) for rax in raxes]
...
"""
   plot_polarization_angle(AΩ)

Plot the histograms of direct and reflected polarization angles.
"""
...
"""
    plot_offaxis_angle(AΩ)

Plot the off-axis angle of direct and reflected events.
"""
...
"""
    plot_acceptance(AΩ::Acceptance, T)

Plot the direct and reflected acceptance of CoRaLS.
"""
...
"""
    plot_differential_spectrum(AΩ::Acceptance, T)

Plot the differential event spectrum of CoRaLS events given
an acceptance and a mission duration.
"""
